...
`;
  ...
  const authLibSrc = readFileSync(join(process.cwd(), 'lib/auth.ts'), 'utf8');
  assert('owner auth uses ownerSecretMatches', /ownerSecretMatches/.test(ownerAuthSrc));
  assert(
    'ownerSecretMatches reads OWNER_KEY or OWNER_RIDDLE',
    /OWNER_RIDDLE/.test(authLibSrc) && /OWNER_KEY/.test(authLibSrc) && /ownerSecretMatches/.test(authLibSrc),
  );
  assert('owner auth allows /proof as a room', /\/proof/.test(ownerAuthSrc));
  assert('owner auth allows /evolution as a room', /\/evolution/.test(ownerAuthSrc));
