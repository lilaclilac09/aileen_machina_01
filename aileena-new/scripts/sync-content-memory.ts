...
import { DECK_LIBRARY_TRACKS } from '../lib/djSetlist';
...
/** Full /sound deck library — lives in `lib/djSetlist.ts` (not DJStation.tsx). */
function parsePlayerTracks(): TrackRow[] {
  return DECK_LIBRARY_TRACKS.map((t) => ({
    id: t.id,
    title: t.title,
    artist: t.artist,
    bpm: t.bpm,
...
Regenerated by \`pnpm sync:content-memory\` (daily GitHub Action + before Dreaming).
...
Review and merge durable facts into \`memories/semantic/\` or \`prompts/\` if needed. Dreaming will pick this up on the next daily run.
`;
}

function main() {
  const setlist = parseSetlist();
  const playerTracks = parsePlayerTracks();
