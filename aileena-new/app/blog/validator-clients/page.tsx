  ...
  return (
    ...
    >
      <article style={{ maxWidth: 900, margin: '0 auto', padding: '64px 32px 120px' }}>
        ...
        <p style={bodyStyle}>
          Five validator implementations target the Solana protocol in 2026. Three are running real stake on mainnet today, one is partial-production, one is testnet. Each one is a different bet about which part of the validator was the bottleneck. The interesting question isn&apos;t which one is fastest — it&apos;s what each team chose <em>not</em> to do, and what that tells you about where the next 10× actually lives.
        </p>
...
{`NIC → net → quic → verify → dedup → pack → bank → poh → shred → store
       ...
       sig check     drop   pack  txn    hash  out   write`}
        ...
        <ul style={listStyle}>
          <li>
            <strong style={strong}>One job per tile.</strong> A verify tile only verifies signatures. A pack tile only orders transactions. There&apos;s no general-purpose scheduler — no thread pool, no work-stealing queue. Each tile is a fixed-function module.
          </li>
          <li>
            <strong style={strong}>Memory is preallocated, never freed.</strong> Tiles allocate all their working memory at startup using huge pages. No <code style={codeStyle}>malloc</code> in the hot path. The arena allocator is the only memory primitive.
          </li>
          <li>
            <strong style={strong}>Tiles never sleep.</strong> Busy-polling, every tile, always. The cost is one core fully consumed per tile; the payoff is zero scheduler jitter.
          </li>
          <li>
            <strong style={strong}>Kernel bypass on the network path.</strong> The <code style={codeStyle}>net</code> tile uses Linux <code style={codeStyle}>AF_XDP</code> in <code style={codeStyle}>drv</code> mode where the NIC supports it — DMA straight to userspace, no syscall per packet, no kernel routing.
          </li>
          <li>
            <strong style={strong}>Zero syscalls in the hot tiles.</strong> Verify, dedup, pack, and poh make zero system calls during normal operation. <code style={codeStyle}>seccomp</code> enforces it: each tile ships with an explicit syscall blacklist.
          </li>
        </ul>
        ...
        <p style={bodyStyle}>
          <strong style={strong}>AF_XDP, in <code style={codeStyle}>drv</code> mode.</strong> The <code style={codeStyle}>net</code> tile binds the NIC&apos;s receive queues directly into userspace memory using Linux&apos;s <code style={codeStyle}>AF_XDP</code> socket family. In <code style={codeStyle}>drv</code> mode the NIC writes packets straight into a userspace ring buffer with DMA, skipping the kernel routing stack, the per-packet syscall, and the copy a normal Linux socket would force. The tile busy-polls the ring on its dedicated core and wakes the kernel only about 20,000 times per second to batch RX/TX completions — small enough to be free, large enough to keep packets flowing. The <code style={codeStyle}>skb</code> fallback exists for NICs without driver-mode support, at the cost of giving up the zero-copy.
        </p>
        ...
        <ul style={listStyle}>
          ...
          <li>
            <strong style={strong}>dedup.</strong> A hash-table membership test, memory-bandwidth-bound on the CPU side. FPGA can win on lookup latency but loses on the cost of getting transactions into FPGA memory and back.
          </li>
          <li>
        ...
        <p style={bodyStyle}>
          Figment cut their primary Solana validator over to Frankendancer at <strong style={strong}>epoch 871</strong> on October 30, 2025. The early performance numbers were unambiguously positive:
        </p>
        ...
        <p style={bodyStyle}>
          The takeaway: Frankendancer ships measurable rewards <em>today</em>, on commodity hardware, with no FPGA in the loop. And the pack-tile scheduling — the single most aggressively un-FPGA-friendly part — is doing most of the work.
        </p>
        ...
        <p style={bodyStyle}>
          The validator client question for 2026 isn&apos;t <em>which one is fastest</em> — it&apos;s <em>which architecture absorbs the next protocol change without a rewrite</em>. Agave and Firedancer both clear that bar, and Sig is betting it can do so for the read path. FPGA validators, in the &quot;replace the whole machine with silicon&quot; sense, don&apos;t. The math hasn&apos;t changed since Jump first ran the numbers, and they&apos;re the team most qualified to know.
        </p>
        ...
        <p style={bodyStyle}>
          The pattern is the same across every row: each step keeps the rest of the CPU validator intact. Even the long-horizon FPGA work is a <em>tile replacement</em>, not a re-platforming. That&apos;s exactly what the tile architecture was designed to allow.
        </p>
