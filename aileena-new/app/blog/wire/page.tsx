...
`}</code></pre>
        ...
        <p style={bodyStyle}>
          A Solana RPC node isn&apos;t separate software. It&apos;s the same agave validator binary, started with <code style={codeStyle}>--full-rpc-api</code> and (usually) <code style={codeStyle}>--no-voting</code>. It receives shreds through turbine like any other validator, reconstructs blocks, replays every transaction, holds the full set of accounts in memory, and exposes the JSON-RPC server. The reason it exists at all is that ordinary validators don&apos;t expose <code style={codeStyle}>getAccountInfo</code> to the public — they&apos;re too busy doing consensus to serve thousands of read queries.
        </p>
        <p style={bodyStyle}>
          This architecture has a consequence developers feel directly: the read path inherits every constraint of the write path. The RPC node carries the same heavy memory footprint, the same expensive disk layout, the same tight coupling to the leader&apos;s tick rate. It&apos;s also why public RPCs throttle so aggressively (a runaway dashboard could starve a real validator&apos;s replay), and why dedicated nodes cost thousands per month (you&apos;re renting a full validator&apos;s worth of resources just for reads). The 2026 architectural fight in RPC is whether this coupling should exist at all — projects like FluxRPC are betting it shouldn&apos;t.
        </p>

        <SectionLabel>JSON-RPC: The Polling Interface</SectionLabel>
        <p style={bodyStyle}>
          The default way to talk to Solana is JSON-RPC over HTTP. POST a JSON envelope, get a JSON envelope back. The protocol has four fields: <code style={codeStyle}>jsonrpc</code> (always &quot;2.0&quot;), <code style={codeStyle}>id</code> (yours to echo), <code style={codeStyle}>method</code> (the call), and <code style={codeStyle}>params</code> (a positional array). The response always carries either <code style={codeStyle}>result</code> or <code style={codeStyle}>error</code>.
        </p>
...
for await (const update of stream) {
  ...
  if (update.slot)        handleSlot(update.slot);
        ...
        <p style={bodyStyle}>
          The trade-off is speed vs. certainty. A market maker reads at <code style={codeStyle}>processed</code> because 12 seconds of latency to <code style={codeStyle}>finalized</code> is fatal in a quoting loop. A wallet showing &quot;your transfer succeeded&quot; uses <code style={codeStyle}>confirmed</code>, because that&apos;s the threshold where the user has near-certainty. An exchange crediting a deposit waits for <code style={codeStyle}>finalized</code>, because the cost of crediting a reverted deposit is far higher than 13 seconds of latency.
        </p>
        <p style={bodyStyle}>
          The non-obvious bit: at the Yellowstone gRPC level, the server can buffer updates per slot at the <code style={codeStyle}>confirmed</code> and <code style={codeStyle}>finalized</code> levels — it holds the update until the slot reaches that threshold, then releases it. The Triton-recommended optimization is to subscribe at <code style={codeStyle}>processed</code> and do the buffering client-side: you get the lowest latency for time-sensitive code paths and keep control over how forks are handled.
        </p>

        <SectionLabel>Forks: The Bug Most Devs Never See Until Production</SectionLabel>
        <p style={bodyStyle}>
          Solana has forks. The chain isn&apos;t a single chain in real time; at any given moment there are short-lived alternative branches that haven&apos;t been resolved yet. A slot that looks live at <code style={codeStyle}>processed</code> can vanish if the network picks a different fork. The slot doesn&apos;t just &quot;not finalize&quot; — it transitions to <code style={codeStyle}>SLOT_DEAD</code>, and every transaction it contained is gone. Never happened. Never charged. Never landed.
        </p>
        <p style={bodyStyle}>
          The Yellowstone <code style={codeStyle}>slots</code> subscription is how you become fork-aware. It emits a status for every slot the validator has seen: <code style={codeStyle}>SLOT_FIRST_SHRED_RECEIVED</code> (first shred arrived), <code style={codeStyle}>SLOT_CREATED_BANK</code> (execution bank instantiated), <code style={codeStyle}>SLOT_COMPLETED</code> (all shreds received, bank fully built), then eventually <code style={codeStyle}>SLOT_DEAD</code> or <code style={codeStyle}>finalized</code>. Fork-aware code buffers account updates by slot and only releases them once the slot reaches the commitment level the application needs. Code that skips this works fine in development and breaks under production volatility — the failure mode is &quot;the database says the trade happened but it didn&apos;t,&quot; and it&apos;s extraordinarily hard to debug if you weren&apos;t expecting it.
        </p>
        ...
        <p style={bodyStyle}>
          The whole stack, top to bottom: validators run consensus, broadcasting shreds through Turbine to every other validator in 2-3 hops. Shreds reconstruct into blocks, blocks belong to slots, and slots progress from processed to confirmed to finalized. An RPC node is just a validator that happens to expose a JSON-RPC server. The three call styles — JSON-RPC, WebSocket, gRPC — differ by polling vs. push, by JSON vs. protobuf, and by how much of the parsing the SDK can move off your application thread.
        </p>
        <p style={bodyStyle}>
          Every &quot;fast&quot; story in the Solana infrastructure landscape is really a story about <em>skipping a layer</em>. Jito ShredStream skips the block-construction wait by reading shreds. Triton co-location skips depth in the turbine tree by sitting at depth 1. LaserStream SDKs skip the JS event loop by doing protobuf work in Rust. Lantern skips the network entirely by caching account state in a local sidecar process. Priority fees, Jito bundles, Helius Sender, and Cascade all skip the contested public TPU queue by buying inclusion bandwidth a different way.
        </p>
        ...
        <p style={bodyStyle}>
          What the API documentation describes — <code style={codeStyle}>getAccountInfo</code>, <code style={codeStyle}>sendTransaction</code>, <code style={codeStyle}>accountSubscribe</code> — is the application surface. What actually happens is a four-layer journey through validators, shreds, the leader schedule, and consensus. You can ship code without knowing any of this. But the moment something breaks — a transaction that won&apos;t land, an account update that&apos;s 12 seconds late, a market maker whose cancels keep losing the race — every layer suddenly matters. The companion piece, <Link href="/blog/rpc" style={{ color: 'rgba(0,255,234,0.75)' }}>The RPC Layer That Cut the Cord</Link>, takes the same machinery and reads it as a market: who sells what, and why.
        </p>
