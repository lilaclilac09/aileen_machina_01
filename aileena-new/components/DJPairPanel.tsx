...
export default function DJPairPanel({
  ...
  const mixableLib = useMemo(() => library.filter(isMixableTrack), [library]);
  const canPair = mixableLib.length >= 2;
  const pairs = useMemo(() => {
    if (!selected || !isMixableTrack(selected) || !canPair) return [];
    return recommendPairs(selected, mixableLib, { hardTechnoBias: hard, limit: 3 }).slice(0, 3);
  }, [selected, mixableLib, hard, canPair]);

  return (
    <div
      data-testid="dj-pair-panel"
      data-hard-techno={hard ? 'true' : 'false'}
      data-mixable-count={String(mixableLib.length)}
      style={{
        margin: '0 0 10px',
        padding: '12px',
        borderRadius: 8,
    ...
    >
      ...
      {!canPair ? (
        <div style={{ marginTop: 8 }}>
          <SystemToast testId="dj-pair-need-two" quiet>
            Need two tracks.
          </SystemToast>
        </div>
      ) : pairs.length > 0 ? (
        <details data-testid="dj-pair-details">
          ...
          >
            Suggestions for “{selected?.title}”
          </summary>
          {pairs.map((p) => (
            <div
              key={p.trackId}
