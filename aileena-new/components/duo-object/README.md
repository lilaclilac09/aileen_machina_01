# /sound/duo — iPhone Duo as a bedside object

Object first. Not a foldable spec sheet. Not a phone UI demo.

## Modes

| pose | how | intent |
|------|-----|--------|
| tent | default · click empty desk · key `1` | Stands like a speaker + frame. Album on the front. Light leaks hinge / back. |
| hold | click the device · key `2` | Lifts off the desk. Brightness and density rise together. Preview play only. |
| rest | double-click the device · key `3` | Almost closed. Live clock + low light. Night lamp, not a lock screen. |

## Light (the third screen)

| light | follows |
|-------|---------|
| music | cover hue |
| clock | warm red |
| notify | short pulse, no badge |
| charge | slow breath |

## Skins to pick

heat · kiln · ember · aurora · signal

Same object. Five rooms. Owner picks one.

## Do not

- This is not the mix. Play here only drives local progress + light.
- Real decks stay on `/sound`.
- Visual kiln stays on `/`.
