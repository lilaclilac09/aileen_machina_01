/**
 * Keyword + TF-IDF search over the build-time article index.
 *
 * Loaded at module init from agentArticleIndex.json. Edge-runtime safe
 * (no fs / path; only a JSON import).
 */

import indexJson from './agentArticleIndex.json';

export type SearchHit = {
  slug: string;
  title: string;
  section: string;
  snippet: string;
  score: number;
};

type RawChunk = {
  slug: string;
  title: string;
  sectionHint: string;
  text: string;
};

type RawIndex = {
  generatedAt: string;
  chunks: RawChunk[];
};

const STOPWORDS = new Set([
  'a',
  'the',
  'is',
  'of',
  'in',
  'for',
  'to',
  'that',
  'it',
  'on',
  'with',
  'as',
  'by',
  'an',
  'and',
  'or',
  'but',
  'this',
  'was',
  'are',
  'be',
  'at',
  'from',
  'has',
  'have',
  'will',
  'would',
  'could',
  'should',
]);

function tokenize(s: string): string[] {
  return s
    .toLowerCase()
    .split(/[^a-z0-9]+/)
    .filter((t) => t.length > 1 && !STOPWORDS.has(t));
}

// ── Build the in-memory index at module load ─────────────────────────────────
const raw = indexJson as RawIndex;
const CHUNKS = raw.chunks;
const N = CHUNKS.length;

// Token frequency per chunk + global document-frequency.
const chunkTokens: Array<Map<string, number>> = new Array(N);
const df: Map<string, number> = new Map();

for (let i = 0; i < N; i++) {
  const tf = new Map<string, number>();
  const tokens = tokenize(CHUNKS[i].text + ' ' + CHUNKS[i].title);
  for (const t of tokens) {
    tf.set(t, (tf.get(t) ?? 0) + 1);
  }
  chunkTokens[i] = tf;
  for (const t of tf.keys()) {
    df.set(t, (df.get(t) ?? 0) + 1);
  }
}

const idf = new Map<string, number>();
for (const [t, freq] of df.entries()) {
  // Smoothed IDF
  idf.set(t, Math.log(1 + (N - freq + 0.5) / (freq + 0.5)));
}

// ── Query ────────────────────────────────────────────────────────────────────
export function searchArticles(query: string, k: number = 3): SearchHit[] {
  const qTokens = tokenize(query);
  if (qTokens.length === 0) return [];

  // Dedup query tokens but track which had highest IDF for snippet centring.
  const uniqQ = Array.from(new Set(qTokens));

  const scored: Array<{ idx: number; score: number }> = [];
  for (let i = 0; i < N; i++) {
    const tf = chunkTokens[i];
    let score = 0;
    for (const t of uniqQ) {
      const f = tf.get(t);
      if (!f) continue;
      const w = idf.get(t) ?? 0;
      // tf-idf with a sublinear tf damper
      score += w * (1 + Math.log(f));
    }
    if (score > 0) scored.push({ idx: i, score });
  }

  scored.sort((a, b) => b.score - a.score);

  // Sort query tokens by IDF descending so we centre snippets on the most
  // informative term.
  const qByIdf = [...uniqQ].sort(
    (a, b) => (idf.get(b) ?? 0) - (idf.get(a) ?? 0),
  );

  const hits: SearchHit[] = [];
  for (const { idx, score } of scored.slice(0, k)) {
    const c = CHUNKS[idx];
    hits.push({
      slug: c.slug,
      title: c.title,
      section: c.sectionHint,
      snippet: makeSnippet(c.text, qByIdf),
      score: Number(score.toFixed(4)),
    });
  }
  return hits;
}

function makeSnippet(text: string, qByIdf: string[]): string {
  const WINDOW = 220;
  if (text.length <= WINDOW) return text;

  // Find the first occurrence of any high-IDF query token.
  const lower = text.toLowerCase();
  for (const t of qByIdf) {
    const i = lower.indexOf(t);
    if (i !== -1) {
      const half = Math.floor(WINDOW / 2);
      let start = Math.max(0, i - half);
      let end = Math.min(text.length, start + WINDOW);
      // Re-align start if end clipped at text length.
      if (end - start < WINDOW) start = Math.max(0, end - WINDOW);
      // Trim to word boundaries.
      if (start > 0) {
        const sp = text.indexOf(' ', start);
        if (sp !== -1 && sp - start < 40) start = sp + 1;
      }
      let snip = text.slice(start, end).trim();
      if (start > 0) snip = '…' + snip;
      if (end < text.length) snip = snip + '…';
      return snip;
    }
  }
  return text.slice(0, WINDOW).trim() + '…';
}
