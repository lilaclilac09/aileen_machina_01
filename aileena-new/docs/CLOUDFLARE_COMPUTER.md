...
- just-bash is not Linux. Owner Linux is the bound computerd container (`demo container` / `container <cmd>`). `pnpm` still does not run in worker-shell. Local `wrangler dev` needs Docker TPROXY (`proxy-everything`); a kernel without `xt_TPROXY` can bind the image and still fail live exec. Production Workers Containers do not use that sidecar.
